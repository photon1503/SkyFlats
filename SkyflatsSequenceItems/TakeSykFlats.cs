...
namespace Photon.NINA.Skyflats {
    ...
    [ExportMetadata("Description", "Capture sky flats when the SQM is in the defined range")]
    [ExportMetadata("Icon", "Plugin_Skyflats_SVG")]
    [ExportMetadata("Category", "SkyFlats")]
    [Export(typeof(ISequenceItem))]
    [Export(typeof(ISequenceContainer))]
    [JsonObject(MemberSerialization.OptIn)]
    public partial class TakeSkyFlats : SequentialContainer, IImmutableContainer {
        ...
        public TakeSkyFlats(IProfileService profileService,
        ...
        private double sqmStart;

        [JsonProperty]
        public double SQMStart {
            get => sqmStart;
            set {
                sqmStart = value;
        ...
        private double sqmEnd;

        [JsonProperty]
        public double SQMEnd {
            get => sqmEnd;
            set {
                sqmEnd = value;
                RaisePropertyChanged();
        ...
        public override async Task Execute(IProgress<ApplicationStatus> progress, CancellationToken token) {
            // check for SQM condition
            if (weatherDataMediator.GetInfo().SkyQuality < SQMStart) {
                return;
            }
            ...
            try {
                ...
                if (loop.CompletedIterations >= loop.Iterations) {
                    Logger.Warning($"The {nameof(TakeSkyFlats)} progress is already complete ({loop.CompletedIterations}/{loop.Iterations}). The instruction will be skipped");
                    throw new SequenceItemSkippedException($"The {nameof(TakeSkyFlats)} progress is already complete ({loop.CompletedIterations}/{loop.Iterations}). The instruction will be skipped");
                }
        ...
        /// <remarks></remarks>
        /// <returns></returns>
        private async Task TakeSkyFlatsSeq(SkyFlatExposureDetermination exposureDetermination, IProgress<ApplicationStatus> mainProgress, CancellationToken token) {
            var dsoContainer = RetrieveTarget(this.Parent);
